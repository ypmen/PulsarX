...
void PulsarPlot::plot(const DedispersionLite &dedisp, const ArchiveLite &archive, GridSearch &gridsearch, std::map<std::string, std::string> &obsinfo, int id, const string &rootname)
{
    ...
    plt::plot(vph, vp);
    //plt::errorbar(vector<double>{1}, vector<double>{0}, vector<double>{0}, vector<double>{dmsmear_phase}, {{"color", "red"}});
    ...
    plt::pcolormesh(vph.size(), vf.size(), mxfph);
    //plt::ylabel("Index");
    plt::axis("auto");
    plt::tick_params({{"which", "both"}, {"direction", "in"}, 
                      {"bottom", "on"}, {"top", "on"}, {"left", "on"}, {"right", "on"}, 
    ...
    plt::pcolormesh(vph.size(), vt.size(), mxtph);
    //plt::ylabel("Index");
    plt::axis("auto");
    plt::tick_params({{"which", "both"}, {"direction", "in"}, 
                      {"bottom", "on"}, {"top", "on"}, {"left", "on"}, {"right", "on"}, 
